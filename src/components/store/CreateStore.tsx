...
const CreateStore: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const response = await post<Store>('/stores/', store);
            if (response.code === 200) {
                showToastSuccess('Store created successfully');
                navigate('/stores');
            } else {
                showToastError(`Failed to create store: ${response.message}`);
            }
        } catch (error) {
            console.error('Failed to create store:', error);
            showToastError(`Failed to create store: ${error}`);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto p-8 bg-white shadow-lg rounded-lg">
            <h2 className="text-3xl font-bold mb-8 text-gray-800">Tạo Store Mới</h2>
